...
impl Cpu {
    pub fn get_operand_address(&mut self, mode: AddressingMode, pc: u16) -> (u16, bool) {
        match mode {
            ...
            AddressingMode::ZeroPage => { // Zero page address (0x0000-0x00FF)
                let address = self.read(pc) as u16;
                (address, false)
            },
            AddressingMode::ZeroPageX => { // Zero page address + X register (wraps within 0x0000-0x00FF)
                let base = self.read(pc);
                let address = base.wrapping_add(self.x) as u16;
                (address, false)
            },
            AddressingMode::ZeroPageY => { // Zero page address + Y register (wraps within 0x0000-0x00FF)
                let base = self.read(pc);
                let address = base.wrapping_add(self.x) as u16;
                (address, false)
            }
            AddressingMode::Relative => { // 8-bit signed offset (wraps at $FFFF)
                let offset = self.read(pc) as i8;
                let address = self.pc.wrapping_add(offset as u16);
                (address, (self.pc & 0xFF00) != (address & 0xFF00))
            }
            AddressingMode::Absolute => { // Full 16-bit address
                let address = self.read_u16(pc);
                (address, false)
            },
            AddressingMode::AbsoluteX => { // Full 16-bit address + X register (wraps at $FFFF)
                let base = self.read_u16(pc);
                let address = base.wrapping_add(self.x as u16);
                (address, (base & 0xFF00) != (address & 0xFF00))
            },
            AddressingMode::AbsoluteY => { // Full 16-bit address + Y register (wraps at $FFFF)
                let base = self.read_u16(pc);
                let address = base.wrapping_add(self.y as u16);
                (address, (base & 0xFF00) != (address & 0xFF00))
            },
            AddressingMode::Indirect => { // Full 16-bit address read from pointer address (only used by JMP)
            ...
            AddressingMode::IndirectX => { // Full 16-bit address read from pointer address + X register (wraps within 0x0000-0x00FF)
                let base = self.read(pc);
            ...
            AddressingMode::IndirectY => { // Full 16-bit address read from pointer address + Y register (wraps within 0x0000-0x00FF)
